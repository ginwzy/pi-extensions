...
import fastModeExtension, {
  ...
  shouldApplyFastMode,
  withFastServiceTier,
} from "../src/modules/gpt-fast-mode/extension.js";
...
test("patches only supported GPT payloads", () => {
  ...
  expect(shouldApplyFastMode({ provider: "OpenAI", id: "gpt-5.6-terra" }, { model: "gpt-5.6-terra" })).toBe(true);
...
test("loads configured shortcuts and toggles payload patching", async () => {
  ...
  try {
    ...
    expect(pi.emittedEvents.at(-1)).toEqual({
      name: FAST_MODE_STATE_EVENT,
      payload: { enabled: true, active: true, provider: TARGET_PROVIDER, modelId: TARGET_MODEL },
    });
    expect(payloadHook({ payload: { model: TARGET_MODEL, store: false } }, ctx)).toEqual({
      model: TARGET_MODEL,
      store: false,
    ...
    expect(pi.emittedEvents.at(-1)).toEqual({
      name: FAST_MODE_STATE_EVENT,
      payload: { enabled: false, active: false, provider: TARGET_PROVIDER, modelId: TARGET_MODEL },
    });
