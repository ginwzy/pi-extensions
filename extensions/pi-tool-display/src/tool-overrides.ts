...
function renderEditDisplayCall(
  ...
  const previewData = resolvePendingDiffPreview(
    ...
    previewKey,
    () => buildPendingEditPreviewData(args, cwd),
  );
...
export function registerToolDisplayOverrides(
  ...
  const renderSearchToolResult = (
    ...
    const config = getConfig();
    return renderSearchResult(result as never, options, config, theme, unitLabel, result.details as never, pluralLabel);
  };
  ...
  registerIfOwned("write", () => {
    ...
    renderResult(result, options, theme, context) {
      ...
      return renderWriteDiffResult(
        ...
        {
          expanded: options.expanded,
          filePath: getToolPathArg(context?.args) ?? undefined,
