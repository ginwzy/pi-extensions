...
export const FAST_MODE_STATE_EVENT = "pi-gpt-fast-mode:state";
...
export function modelKey(model: PiModel): string {
  return `${model.provider?.toLowerCase()}/${model.id}`;
}
...
function currentModel(ctx: unknown): PiModel | undefined {
  return (ctx as { model?: PiModel } | undefined)?.model;
}

function isSupportedModelContext(ctx: unknown): boolean {
  return isSupportedModel(currentModel(ctx));
}
...
export default function fastModeExtension(pi: ExtensionAPI): void {
  ...
  function publishState(ctx: unknown): void {
    const model = currentModel(ctx);
    pi.events.emit(FAST_MODE_STATE_EVENT, {
      enabled,
      active: enabled && isSupportedModel(model),
